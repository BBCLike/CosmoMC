    MODULE BBC
    !Module for handling BBC supernova data
    ...
    !         beta). This is about 150MFLOPs for the BBC sample. The
    ...
    !   mbetoule, Dec 2013: adaptation to the BBC sample
    !   AL, Mar 2014: updates for latest CosmoMC structure
    !   AL, June 2014: updated BBC_marginalize=T handling so it should work (also default BBC.ini)
    USE CosmologyTypes
    USE settings
    use CosmoTheory
...
140 WRITE(*,*) 'Error reading in input data with: ',inline
    ...
    !------------------------------------------------------------
    SUBROUTINE match_BBC_absdist_indices( sndata, snabsdist )
    IMPLICIT NONE
    ...
    END SUBROUTINE match_BBC_absdist_indices
    ...
    IF ( has_absdist ) THEN
        ...
        CALL read_BBC_absdist_data( file_unit, nlines, nabsdist, snabsdist )
        CLOSE( file_unit )
        CALL match_BBC_absdist_indices( sndata, snabsdist )
    ENDIF
    ...
    IF (Feedback > 2) THEN
        WRITE(*,'(" BBC pec z: ",F6.3)') pecz
        WRITE(*,'(" BBC default sigma int: ",F6.3)') idisp_zero
        DO i=1, max_idisp_datasets
            IF ( idispdataset(i)) &
                WRITE(*,'(" BBC sigma int for dataset ",I2,": ",F6.3)') &
                i-1,intrinsicdisp(i)
        END DO
        IF (has_absdist) THEN
...
500 WRITE(*,*) 'Error reading ' // data_file
    ...
    ! ReadBBCDataset must have been read before calling this
    !------------------------------------------------------------
    SUBROUTINE BBC_prep
    IMPLICIT NONE
    ...
    CHARACTER(LEN=*), PARAMETER :: datafile = 'data/BBC_data.dat'
...
500 WRITE(*,*) 'Error reading ' // datafile
    ...
    !------------------------------------------------------------
    ! Routine for calculating the log-likelihood of the BBC
    ! data.  You _have_ to call this just after calling CAMB
    ! with the model you want to evaluate against.   It's assumed
    ! that you have called read_BBC_dataset and BBC_prep before
    ! trying this.
    !
    ! Arguments:
    ...
    FUNCTION  BBC_alpha_beta_like(alpha, beta,  lumdists)
    real(mcp) :: BBC_alpha_beta_like
    ...
    ENDIF
    BBC_alpha_beta_like = chisq / 2  !Negative log likelihood

    IF (Feedback > 1 .and. .not. BBC_marginalize) THEN
        IF (Feedback > 2) THEN
            IF (twoscriptmfit) THEN
                WRITE(*,'(" BBC alpha: ",F7.4," beta: ",F7.4," scriptm1: ",F9.4, "scriptm2: ",F9.4)') &
                    alpha,beta,(amarg_B*amarg_F-amarg_C*amarg_D)/tempG,&
                    (amarg_C*amarg_E-amarg_B*amarg_D)/tempG
            ELSE
                WRITE(*,'(" BBC alpha: ",F7.4," beta: ",F9.4," scriptm: ",F9.4)') &
                    alpha,beta,-amarg_B/amarg_E
            ENDIF
        END IF
        WRITE(*,'(" BBC chi2: ",F7.2," for ",I5," SN")') chisq,nsn
    ENDIF
    ...
    end FUNCTION  BBC_alpha_beta_like
    ...
    if (BBC_marginalize) then
        ...
        do grid_i = 1, BBC_int_points
            alpha = alpha_grid(grid_i)
            beta=beta_grid(grid_i)
            BBC_marge_grid(grid_i) = BBC_alpha_beta_like(alpha, beta, lumdists)
        end do

        grid_best = minval(BBC_marge_grid,mask=BBC_marge_grid/=logZero)
        BBC_LnLike =  grid_best - log(sum(exp(-BBC_marge_grid + grid_best),  &
            mask=BBC_marge_grid/=logZero)*BBC_step_width_alpha*BBC_step_width_beta)
        IF (Feedback > 1) THEN
            WRITE(*,'(" BBC best logLike ",F7.2,", marge logLike: ",F7.2," for ",I5," SN")') grid_best, BBC_LnLike,nsn
        end if
    else
        alpha=DataParams(1)
        beta=DataParams(2)
